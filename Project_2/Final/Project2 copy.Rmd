...
df2 <- eventReactive(c(input$selectdrg_definition_3, input$average_total_payments_filter_1), { 
  project <- "https://data.world/brandon-kerbow/s18-edv-project-2-dataset" 
  data.world::set_config(cfg_env("DW_API")) 
  paramQuery <- data.world::qry_sql(
   "   
...
order by drg_definition
   ")
  paramQuery$params <- c(input$average_total_payments_filter_1[1], input$average_total_payments_filter_1[2], input$selectdrg_definition_3[1], input$selectdrg_definition_3[2], input$selectdrg_definition_3[3], input$selectdrg_definition_3[4], input$selectdrg_definition_3[5], input$selectdrg_definition_3[6], input$selectdrg_definition_3[7], input$selectdrg_definition_3[8], input$selectdrg_definition_3[9], input$selectdrg_definition_3[10])
  data.world::query(paramQuery, dataset = project)
}) 
```

```{r}
df3 <- eventReactive(c(input$selectdrg_definition_3, input$yDataMin__3_, input$yDataMax__3_,input$average_total_payments_filter_1), { 
...
dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      menuItem("Boxplot with Cities", tabName = "boxplot1", icon = icon("dashboard"))
    ),
    sidebarMenu(
      menuItem("Boxplot with Disease", tabName = "boxplot3", icon = icon("dashboard"))
    )
  ),
  dashboardBody(
    tabItems(
      # 1. "Boxplot with Cities" tab content
      tabItem(tabName = "boxplot1",
        source("BoxPlot1UI.R",local = TRUE)$value
      ),
      # 2. "Boxplot with Diseases" tab content

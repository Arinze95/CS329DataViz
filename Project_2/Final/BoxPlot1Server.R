...
# Build and Output Throughput Plot ------------------------
  output$BoxPlotPlot__1_ <- renderPlotly({
    plot = df1() %>% ggplot() + 
      geom_boxplot(mapping = aes(x = drg_definition, y=average_total_payments, colour = hospital_referral_region_description)) +
    ...
    if( ! is.na(input$plotWidth__1_) & ! is.na(input$plotHeight__1_))
     ggplotly(plot, tooltip = c("Disease", "Average Total Payments"), session="knitr", width = input$plotWidth__1_, height = input$plotHeight__1_)
    else
      ggplotly(plot, tooltip = c("Disease", "Average Total Payments"), session="knitr") 
  })
# ---------------------------------------------------------

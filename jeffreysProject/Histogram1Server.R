...
# Build and Output Throughput Plot ------------------------
  output$Histogram__1_ <- renderPlotly({
    ...
    if( ! is.na(input$plotWidth__1_) & ! is.na(input$plotHeight__1_))
     ggplotly(plot, tooltip = c("average_total_payments"), session="knitr", width = input$plotWidth__1_, height = input$plotHeight__1_)
    else
      ggplotly(plot, tooltip = c("average_total_payments"), session="knitr") 
  })
# ---------------------------------------------------------

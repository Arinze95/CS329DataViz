dfB <- eventReactive(c(input$selectReg_1, input$incomeFilter_1), { 
...
dfB1 <- eventReactive(c(input$selectReg_1, input$incomeFilter_1, input$yDataMin__2_, input$yDataMax__2_), { 
  if( ! is.na(input$yDataMin__2_) & ! is.na(input$yDataMax__2_)) {
    dfB() %>% dplyr::filter(between(gross_national_income_per_capita_ppp_international, input$yDataMin__2_, input$yDataMax__2_))
  }
  else {
    dfB()
  }

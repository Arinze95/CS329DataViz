...
# Build Download Button and Output DataTable---------------
  output$download__2_ <- downloadHandler(
    ...
    content = function(fname){
      write.csv(dfA2(), fname)
    }
  )
  output$table__2_ <- renderDataTable({
    DT::datatable(dfA2() , rownames = FALSE, extensions = list(Responsive = TRUE, FixedHeader = TRUE)
    )
  })
# ---------------------------------------------------------
# Build and Output Throughput Plot ------------------------
  output$Histogram__2_ <- renderPlotly({
    plot = dfA2() %>% ggplot() + 
      ...
      geom_smooth(aes(y=contraceptive_prevalence, x=income_per_person, colour=system_of_government), method = lm, formula = y ~ x, se=FALSE) +
    ...
    if( ! is.na(input$plotWidth__2_) & ! is.na(input$plotHeight__2_))
     ggplotly(plot, tooltip = c("population_in_thousands_total"), session="knitr", width = input$plotWidth__2_, height = input$plotHeight__2_)
    else

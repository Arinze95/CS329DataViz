...
##Insight 1 - Exploring the Number of Children per Woman

The scatterplot shows the negative correlation between the literarcy rate and average number of children per woman. To explore the residuals of this correlation, we see if there is a geographic trend in countries in which the average number of children per women is over or under predicted by the model. We see no such geographic trend but the next tab of the story does show a correlation behind the residuals - the infant mortality rate. Note that on the first tab, the map and scatterplot can be used to filter one another. Also included is a set of the 20 richest countries, which interestingly enough has countries which do not follow the negative trend.

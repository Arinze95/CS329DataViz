...
# Build Download Button and Output DataTable---------------
  output$download__4_ <- downloadHandler(
    ...
    content = function(fname){
      write.csv(dfB1Jeff(), fname)
    }
  )
  output$table__4_ <- renderDataTable({
    DT::datatable(dfB1Jeff() , rownames = FALSE, extensions = list(Responsive = TRUE, FixedHeader = TRUE)
    )
  })
# ---------------------------------------------------------
# Build and Output Throughput Plot ------------------------
  output$Histogram__4_ <- renderPlotly({
    plot = dfB1Jeff() %>% ggplot() + geom_bar(mapping = aes(x=region, y=Average,colour=system_of_government, fill=system_of_government), stat = "identity") +
    ...
    if( ! is.na(input$plotWidth__4_) & ! is.na(input$plotHeight__4_))
     ggplotly(plot, tooltip = c("Expenditure"), session="knitr", width = input$plotWidth__4_, height = input$plotHeight__4_)
    else

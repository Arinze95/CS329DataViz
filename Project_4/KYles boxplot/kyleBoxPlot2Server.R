...
output$download__2_ <- downloadHandler(
  ...
  content = function(fname){
    write.csv(df2kyle(), fname)
  }
)
output$table__2_ <- renderDataTable({
  DT::datatable(df2kyle() , rownames = FALSE, extensions = list(Responsive = TRUE, FixedHeader = TRUE)
  )
})
# ---------------------------------------------------------
# Build and Output Throughput Plot ------------------------
output$BoxPlotPlot__2_ <- renderPlotly({
  plot = df2kyle() %>% ggplot() + 
    geom_boxplot(mapping = aes(x = city, y = avg_total_income_for_zipcode, colour = property_type)) +
    ...
    ggtitle(paste(input$title__2_)) + #ggtitle(paste(Hospital_Regions(), input$title__9_)) +
  ...
  if( ! is.na(input$plotWidth__2_) & ! is.na(input$plotHeight__2_))
    ggplotly(plot, tooltip = c("city", "avg_total_income_for_zipcode"), session="knitr", width = input$plotWidth__2_, height = input$plotHeight__2_)
  else
    ggplotly(plot, tooltip = c("city", "avg_total_income_for_zipcode"), session="knitr") 
})
# ---------------------------------------------------------
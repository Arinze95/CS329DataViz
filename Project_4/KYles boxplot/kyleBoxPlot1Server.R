...
output$BoxPlotPlot__1_ <- renderPlotly({
  plot = df1kyle() %>% ggplot() + 
    geom_boxplot(mapping = aes(x = city, y = nightly_price, colour = property_type)) +
    ...
    ggtitle(paste(input$title__1_)) + #ggtitle(paste(Hospital_Regions(), input$title__9_)) +
  ...
  if( ! is.na(input$plotWidth__1_) & ! is.na(input$plotHeight__1_))
    ggplotly(plot, tooltip = c("city", "nightly_price"), session="knitr", width = input$plotWidth__1_, height = input$plotHeight__1_)
  else
    ggplotly(plot, tooltip = c("city", "nightly_price"), session="knitr") 
})
# ---------------------------------------------------------

...
We had hypothesized that having waterfront access might have some impact on the review score, but we did not anticipate the near universally positive impact it seems to have.

One thing to note is that beach and lake access tend to have higher scores on average than those with pool access, altough there are also more reviews for properties with pool access as opposed to those with lake or beach access, this does indicate that people seem to enjoy access to larger bodies of water during their stay.
...
#Exploring Nightly Prices and Average Income
```{r}
...
#Exploring Review Scores- Histogram and Boxplots


Below are two plots. The default cities for both plots are the six cities which our dataset contained. They can also be individually selected using the input panel. The first of which is a boxplot of review score ratings by city. It can be seen that New York not only has the most variable ratings but also outliers that are the lowest. From just this plot and information, one possible reason is that New York has a generally less affluent population.

The next plot is a histogram of average total income of the zipcode the rental is in. Although it does seem that New York does have many of its population in the first section of the plot, it also has a dominant presence in the latter half as well. Using the zoom tool on the the latter half, it is more apparent. Although New York has a more affluent population, many of the AirBnB rentals available are receiving low scores. This suggests that there are obviously other variables at play than simply whether or not the area is rich or not.

...
# Build Download Button and Output DataTable---------------
  output$download__4_ <- downloadHandler(
    ...
    content = function(fname){
      write.csv(dfB1Brandon(), fname)
    }
  )
  output$table__4_ <- renderDataTable({
    DT::datatable(dfB1Brandon() , rownames = FALSE, extensions = list(Responsive = TRUE, FixedHeader = TRUE)
    )
  })
# ---------------------------------------------------------
# Build and Output Throughput Plot ------------------------
  output$Histogram__4_ <- renderPlotly({
    plot = dfB1Brandon() %>% ggplot() + geom_bar(mapping = aes(x=city, y=Average,colour=allows_pets, fill=allows_pets), stat = "identity") +

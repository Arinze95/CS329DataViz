...
# Build Download Button and Output DataTable---------------
  output$download__3_ <- downloadHandler(
    ...
    content = function(fname){
      write.csv(dfA1Brandon(), fname)
    }
  )
  output$table__3_ <- renderDataTable({
    DT::datatable(dfA1Brandon() , rownames = FALSE, extensions = list(Responsive = TRUE, FixedHeader = TRUE)
    )
  })
# ---------------------------------------------------------
# Build and Output Throughput Plot ------------------------
  output$Histogram__3_ <- renderPlotly({
    plot = dfA1Brandon() %>% ggplot() + geom_bar(mapping = aes(x=city, y=Average, colour=property_type, fill=property_type), stat = "identity") +
